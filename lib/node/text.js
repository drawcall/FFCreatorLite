...
class FFText extends FFNode {
  ...
  toFilter() {
    ...
    const enable = FilterUtils.createFilterEnable({appearTime, duration});
    const options = {
    ...
    const filter = {filter: 'drawtext', options};
    return filter;
  }
  ...
  addInputsAndOutputs(context) {
    ...
    forEach(this.filters, (filter, index) => {
      let inputs = index == 0 ? context.input : this.getIID();
      this.generateNewOID();
      let outputs = this.getOID();
