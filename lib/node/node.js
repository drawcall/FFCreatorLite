...
class FFNode extends FFBase {
  ...
  getIID(k = false) {
    if (this.fIndex === 0) {
      return this.getVID(k);
    } else {
      return this.getOID(k);
    }
  }
  ...
  concatFilters(context) {
    ...
    // 3. scale rotate
    const srFilter = FilterUtils.assembleSRFilter({scale: this.scale, rotate: this.rotate});
    ...
    // 6. x/y overlay
    if (!FilterUtils.getOverlayFromFilters(this.filters)) {
      const xyFilter = FilterUtils.createOverlayFilter(this.x, this.y);
      this.filters.push(xyFilter);
    }

    // 7. add appearTime setpts
    // Because overlay enable is used, remove this
    // this.filters.push(FilterUtils.createSetptsFilter(this.appearTime));
  ...
  addDurationToOverlay() {
    ...
    FilterUtils.addDurationToOverlay({
  ...
  addInputsAndOutputs(context) {
    ...
    forEach(this.filters, (filter, index) => {
      const inputs = this.getIID();
      this.generateNewOID();
      const outputs = this.getOID();

      this.filters[index] = FilterUtils.setInputsAndOutputs({
        filter,
        inputs,
        outputs,
  ...
  addBlend(blend) {
    this.addPreFilter(`blend=all_mode=${blend}`);
  }

  addTBlend(blend) {
    this.addPreFilter(`tblend=all_mode=${blend}`);
  }
