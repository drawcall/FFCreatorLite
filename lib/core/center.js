...
const FFCreatorCenter = {
  ...
  onTask(id, eventName, func) {
    ...
    taskQueue.addListeners(id, eventName, func);
  ...
  eventHandler(result) {
    ...
    // or clone result
    if (eventName !== 'single-progress') {
      cresult = cloneDeep(result);
    }

    // remove task Object
    if (eventName === 'single-error' || eventName === 'single-complete') {
      this.removeTaskObj(id);
      // console.log("taskQueue", taskQueue.getLength(), taskQueue.queue);
    }
  ...
  handlingError({taskObj, error = 'normal'}) {
    ...
    taskObj.state = 'error';
    const result = {id: taskObj.id, error, eventName};
    this.event.emit(eventName, result);
